 ...
 */
export const ERROR_TEMPLATES: Record<ErrorType, Omit<ErrorState, 'timestamp'>> = {
  [ErrorType.NO_DATA]: {
    type: ErrorType.NO_DATA,
    message: 'No data available',
    resolutionHint: 'Configure a query to return EXPLAIN output. Use EXPLAIN (FORMAT JSON) SELECT ... or EXPLAIN SELECT ...',
  },

  [ErrorType.FIELD_NOT_FOUND]: {
    type: ErrorType.FIELD_NOT_FOUND,
    message: 'Plan field not found in data',
    resolutionHint: 'Check the "Plan Field Name" setting in panel options. Verify your query returns a field with EXPLAIN output.',
  },

  [ErrorType.INVALID_FORMAT]: {
    type: ErrorType.INVALID_FORMAT,
    message: 'Invalid EXPLAIN format',
    resolutionHint: 'Ensure your query returns PostgreSQL EXPLAIN output in JSON or text format. Use EXPLAIN (FORMAT JSON) for best results.',
  },

  [ErrorType.PARSE_ERROR]: {
    type: ErrorType.PARSE_ERROR,
    message: 'Unable to parse EXPLAIN text format',
    resolutionHint: 'Try using EXPLAIN (FORMAT JSON) instead of plain text EXPLAIN. If using text format, verify the output is complete and valid.',
  },

  [ErrorType.RENDER_ERROR]: {
    type: ErrorType.RENDER_ERROR,
    message: 'Visualization error',
    resolutionHint: 'Check the browser console for details. This may indicate an invalid plan structure or a bug in the plugin.',
  },

  [ErrorType.CSP_VIOLATION]: {
    type: ErrorType.CSP_VIOLATION,
    message: 'Plugin assets blocked by security policy',
    resolutionHint: 'Contact your Grafana administrator. The plugin requires specific Content Security Policy settings to function.',
  },
};
