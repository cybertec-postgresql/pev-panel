 ...
 */
export function validatePanelOptions(options: Partial<PanelOptions>): PanelOptions {
  ...
  if (typeof validated.planFieldName !== 'string' || validated.planFieldName.trim() === '') {
  ...
  if (typeof validated.fontSize !== 'number' || isNaN(validated.fontSize)) {
    ...
    validated.fontSize = Math.max(
      ...
      Math.min(PANEL_OPTIONS_CONSTRAINTS.MAX_FONT_SIZE, Math.round(validated.fontSize))
    );
  }
